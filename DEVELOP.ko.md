...
#### 이 패키지에서 patch 는 patch 가 아니다

**버전 번호가 "얼마나 바뀌었나"가 아니라 "어느 플랫폼용인가"를 가리킨다.** 두 계열을 한
패키지에 번갈아 올리기 때문이다. 두 버전을 실제로 받아 비교하면 파일 수(226개)도
크기(2.6M)도 같고, 차이는 `CryptoFile.js` 의 `require` 한 줄뿐이다 - 그런데 브라우저에는
`crypto` 모듈이 없으므로 그 한 줄이 동작 여부를 가른다.

그래서 **semver 의 전제가 성립하지 않는다.** "patch 니까 안전하다" 가 통하지 않으므로
`.github/dependabot.yml` 에서 major·minor·patch 를 **모두** 무시한다. 다만 보안 업데이트는
받도록 `version-update:` 접두사를 붙여 두었다 - 아래 위험 때문이다.

#### 이 고정이 안고 있는 위험

브라우저 계열은 **항상 한 버전 뒤처진다.** `latest` 에만 들어간 수정은 다음 `browser`
릴리스를 기다려야 받을 수 있고, 그것이 보안 수정이면 그 사이 우리는 취약한 채로 남는다.
Dependabot 의 보안 알림을 살려 둔 이유가 이것이다 - 그 신호가 와야 별칭이든 포크든 판단할
수 있다.

#### 업스트림이 원래 했어야 하는 것

`package.json` 에 이미 `"browser": { "fs": false }` 가 있으므로, 한 줄만 더하면 될 일이었다.
...
또는 요즘 표준인 조건부 진입점(`exports` 의 `browser`/`node` 조건)을 쓰면 번들러가 알아서
고른다. 어느 쪽이든 버전은 하나로 유지되고 dist-tag 로 계열을 나눌 이유가 없어진다.

우리 쪽에서 Vite 의 `resolve.alias` 로 우회할 수도 있지만, 그 방식은 GramJS 내부 파일
구조에 기대는 것이라 업스트림이 파일을 옮기면 조용히 깨진다. 지금은 고정이 깨질 자리가
더 적다.
