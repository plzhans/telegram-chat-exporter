...
`vite.config.ts` 의 `base` 가 `/telegram-chat-exporter/` 로 **고정**되어 있다.
GitHub Pages 가 저장소 이름을 경로에 끼워 서비스하기 때문이다.

그래서 dev·preview 서버 주소에도 이 접두사가 붙는다(아래 [실행](#실행) 참고).
다른 곳에 올릴 일이 생기면 `vite.config.ts` 의 그 한 줄만 고친다 — 루트 배포면 `'/'` 다.
라우터는 같은 값을 `import.meta.env.BASE_URL` 로 읽으므로 따로 손댈 곳이 없다.
...
```bash
pnpm dev     # http://localhost:5175/telegram-chat-exporter/
```

주소에 `/telegram-chat-exporter/` 가 붙는 건 `base` 가 박혀 있어서다(위 참고).
루트로 들어가도 Vite 가 알아서 넘겨준다.
...
```bash
pnpm build
pnpm preview     # http://localhost:5176/telegram-chat-exporter/
```
...
워크플로는 `pnpm install && pnpm build` 를 그대로 돌린다. 하위 경로는 `vite.config.ts` 에
박혀 있어서 CI 에서 따로 넘기는 게 없다. 그 외에 워크플로가 하는 일은 둘뿐이다.
...
Cloudflare 는 루트 배포라 `vite.config.ts` 의 `base` 를 `'/'` 로 바꾸고 빌드해야 한다.
