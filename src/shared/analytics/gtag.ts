...
/**
 * 앱의 첫 화면(방식 고르기) 방문을 한 번 집계한다.
 *
 * 이 앱은 자기 base(예: `/run/`) 아래에서 직접 뜨므로 `safePath()` 가 그 첫 화면 경로를
 * 돌려준다 — 그 한 번만 보내고, 로그인 뒤 화면(대화방 id 가 든 경로)은 보내지 않는다.
 * 홍보 랜딩은 이 저장소와 **별개로 배포되는 독립 프로젝트**(`landing/`)이고, 자기 방문
 * 집계는 자기 `analytics.js` 로 따로 한다 — 여기서 신경 쓸 것이 아니다.
 *
 * `VITE_GOOGLE_ANALYTICS_ID` 가 없으면(단일 파일 배포 포함) 아무것도 하지 않는다.
 * `ANALYTICS_ENABLED` 는 화면이 외부 연결(구글)을 고지할 근거로도 계속 쓰인다 — CSP 가
 * 실제로 열려 있으므로 그 고지는 참이어야 한다.
 */
