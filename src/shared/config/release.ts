 ...
 * **읽는 쪽이 사는 세계가 다르다.**
 *
 * 1. 앱 화면 - 사용자 **브라우저에서** 돈다(`app.ts` 의 `DOWNLOAD_URL`).
 * 2. 릴리스 워크플로 - **셸에서** 이 파일의 기본값을 읽어 zip 이름을 정한다.
 *
 * (홍보 랜딩도 같은 규칙이 필요하지만, 이제 별개 프로젝트라 자기 사본을 쓴다
 *  — `landing/src/config/release.ts`.)
 *
