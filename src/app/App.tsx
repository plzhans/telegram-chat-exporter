...
function RequireAuth() {
  ...
  // 진입점(로그인 화면)으로 보낸다. 홍보 랜딩은 이 앱과 별개로 배포되므로 여기서 갈 곳이 아니다.
...
const pages: RouteObject[] = [
  /*
    첫 화면이 곧 로그인(방식 고르기) 화면이다.

    홍보 랜딩은 이 앱과 **별개로 배포되는 독립 프로젝트**(`landing/`)라, 여기서 신경 쓸
    것이 없다. 웹 배포든 단일 파일 배포든 이 라우터의 첫 화면은 언제나 `SignIn` 이다 —
    랜딩에서 "시작하기" 를 누르면 진짜 페이지 이동으로 이 앱 진입점에 도착한다.
  */
